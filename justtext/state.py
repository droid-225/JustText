...
class GameState:
    ...
    def load_slot(cls, slot: int):
        ...
        if p.exists():
            ...
            return cls(name=data.get("name", ""), count=int(data.get("count", 0)), gold=(data.get("gold", 0)), current_slot=slot)
    ...
    def save(self):
        ...
        p.write_text(json.dumps({"name": self.name, "count": self.count, "gold": self.gold}))
...
def list_slots():
    ...
    for slot in (1, 2, 3):
        ...
        if p.exists():
            data = json.loads(p.read_text())
            entries.append({"slot": slot, "name": data.get("name", ""), "count": int(data.get("count", 0)), "gold" = (data.get("gold", 0))})
        else:
